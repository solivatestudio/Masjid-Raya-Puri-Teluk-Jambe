...
export default function KeuanganPage() {
  ...
  useEffect(() => { fetchData(); }, [filterType, filterCategory, startDate, endDate]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-slate-400">Terakhir diperbarui:</span>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <SummaryCard
            ...
            subtitle="Total all-time pemasukan - pengeluaran"
        ...
        <div className="bg-white rounded-3xl border border-slate-100 card-shadow p-6">
          ...
          <div className="flex items-end gap-3 h-40">
            {monthlyData.map((m) => {
              ...
              const pemasukanH = (m.pemasukan / maxVal) * 120;
              const pengeluaranH = (m.pengeluaran / maxVal) * 120;
              return (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                  <div className="flex flex-col-reverse items-center w-full gap-0.5">
                    <div className="w-full bg-amber-100 rounded-t" style={{ height: `${pengeluaranH}px` }} title={`Pengeluaran: ${formatRupiah(m.pengeluaran)}`} />
                    <div className="w-full bg-emerald-500 rounded-t" style={{ height: `${pemasukanH}px` }} title={`Pemasukan: ${formatRupiah(m.pemasukan)}`} />
                  </div>
        ...
        <form onSubmit={handleSubmit} className="bg-emerald-50/60 border border-emerald-100 rounded-2xl p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
              ...
              <select value={formType} onChange={(e) => { setFormType(e.target.value as any); setFormCat(e.target.value === 'Pemasukan' ? 'Infaq Umum' : 'Operasional'); }}
            ...
            <div>
              ...
              <input type="text" required placeholder="Deskripsi transaksi" value={formDesc} onChange={(e) => setFormDesc(e.target.value)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" required min="1000" step="1000" placeholder="Rp" value={formAmount} onChange={(e) => setFormAmount(e.target.value)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none" />
            </div>
            <div>
