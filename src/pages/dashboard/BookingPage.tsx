...
export default function BookingPage() {
  ...
  const getBookingsForDate = (date: string) => {
    return bookings.filter(b => b.date === date && b.status === 'approved');
  };
    ...
    <div className="space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <SummaryCard title="Booking Terdekat" value={summary.nearestBooking ? formatDateShort(summary.nearestBooking.date) : '-'}
            subtitle={summary.nearestBooking ? `${summary.nearestBooking.name} — ${summary.nearestBooking.purpose}` : 'Tidak ada'} icon={<Users className="w-5 h-5" />} />
        </div>
      )}
        ...
        <form onSubmit={handleCreate} className="bg-emerald-50/60 border border-emerald-100 rounded-2xl p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <input type="text" required placeholder="Nama lengkap" value={formName} onChange={(e) => setFormName(e.target.value)}
            ...
            <div>
              ...
              <input type="text" required placeholder="081234567890" value={formWhatsapp} onChange={(e) => setFormWhatsapp(e.target.value)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" required placeholder="Contoh: Walimah Nikah" value={formPurpose} onChange={(e) => setFormPurpose(e.target.value)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-emerald-900 mb-1">Jam Mulai</label>
              <input type="time" required value={formTimeStart} onChange={(e) => setFormTimeStart(e.target.value)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-emerald-900 mb-1">Jam Selesai</label>
              <input type="time" required value={formTimeEnd} onChange={(e) => setFormTimeEnd(e.target.value)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-[10px] font-bold text-emerald-900 mb-1">Catatan Tambahan</label>
              <input type="text" placeholder="Opsional" value={formNotes} onChange={(e) => setFormNotes(e.target.value)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-2 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none" />
            </div>
          </div>
          <div className="flex justify-end">
            <button type="submit"
              className="bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold px-6 py-2 rounded-xl transition cursor-pointer">
              Simpan Booking
            </button>
          </div>
        </form>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 card-shadow p-5">
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => { if (calMonth === 0) { setCalMonth(11); setCalYear(calYear - 1); } else { setCalMonth(calMonth - 1); } }}
              className="text-xs font-bold text-slate-600 hover:text-emerald-700 cursor-pointer px-2 py-1">‹</button>
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day}
                  className={`relative p-1.5 rounded-lg text-xs font-semibold cursor-default ${
                    isApproved ? 'bg-emerald-100 text-emerald-800' : isToday ? 'bg-amber-50 text-amber-800' : 'text-slate-700 hover:bg-slate-50'
                  }`}
                  title={dayBookings.length > 0 ? `${dayBookings.map(b => b.name).join(', ')}` : ''}
                >
                  {day}
            ...
            {['Semua', 'pending', 'approved', 'rejected'].map((s) => (
              <button key={s} onClick={() => setFilterStatus(s)}
                className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition cursor-pointer ${
                  filterStatus === s ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-600 hover:text-emerald-700'
                }`}>
                        ...
                        {b.status === 'pending' && (
                          <div className="flex items-center gap-1.5 justify-center">
                            <button onClick={() => handleStatus(b.id, 'approved')}
                              className="p-1.5 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded-lg transition cursor-pointer" title="Approve">
                              <CheckCircle className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleStatus(b.id, 'rejected')}
                              className="p-1.5 bg-rose-100 hover:bg-rose-200 text-rose-700 rounded-lg transition cursor-pointer" title="Reject">
