...
export default function KajianPage() {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (
      !(await showConfirm(`Hapus kajian "${title}"?`, {
        title: "Hapus Kajian",
        confirmLabel: "Hapus",
        tone: "danger",
      }))
      ...
      return;
    await fetch(`/api/admin/kajian/${id}`, {
      method: "DELETE",
      credentials: "include",
      cache: "no-store",
    });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Kelola jadwal kajian rutin dan kegiatan dauroh
          </p>
        </div>
        <Link
          href="/dashboard/kajian/new"
      ...
      <DataTable<Kajian>
        ...
        columns={[
          ...
          },
          {
            ...
            render: (k) => (
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-bold ${k.category === "Dakwah" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}
              >
                {k.category}
              </span>
            ),
          },
          {
            ...
            header: "Tanggal",
            className: "w-32",
            render: (k) => <span className="text-xs">{k.date_label}</span>,
          },
          {
            key: "time",
            header: "Waktu",
            className: "w-32",
            render: (k) => (
              <span className="text-xs text-slate-600">{k.time_label}</span>
            ),
          },
          {
            key: "pic",
            header: "PIC",
            className: "w-24",
            render: (k) => <span className="text-xs">{k.pic_name || "—"}</span>,
          },
          {
            key: "actions",
            header: "",
