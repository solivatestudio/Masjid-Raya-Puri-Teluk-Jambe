  ...
  const handleDelete = async (id: string, title: string) => {
    if (
      !(await showConfirm(
        ...
        { title: "Hapus Artikel", confirmLabel: "Hapus", tone: "danger" },
      ))
    )
      return;
    ...
    if (res.ok) fetchData();
    else await showAlert("Gagal menghapus", { title: "Gagal" });
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Kelola artikel & berita untuk website masjid
          </p>
        </div>
        <Link
          href="/dashboard/articles/new"
          className="bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition flex items-center gap-1.5"
        >
      ...
      <DataTable<Article>
        ...
        columns={[
          ...
          },
          {
            ...
            render: (a) => (
              <span className="text-xs text-slate-500">{a.views_count}</span>
            ),
          },
          {
            key: "updated",
