  ...
  const fetchData = async () => {
    try {
      ...
      setData({
        ...summary,
        recentVisits: recent,
        daily7Days: fillDailyViews(summary.daily7Days || []),
      });
    ...
    <div className="space-y-6">
      ...
      {data && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <SummaryCard title="Active Visitors (5 Menit)" value={String(data.activeVisitors)} icon={<Activity className="w-5 h-5" />} variant="primary" />
          <SummaryCard title="Pageviews (7 Hari)" value={String(data.total7Days)} icon={<Eye className="w-5 h-5" />} />
          <SummaryCard title="Unique Visitors" value={String(data.uniqueVisitors)} icon={<Users className="w-5 h-5" />} />
