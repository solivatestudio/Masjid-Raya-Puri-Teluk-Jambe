...
export default function KeuanganPage() {
  ...
  const handleDelete = async (id: string) => {
    if (
      !(await showConfirm("Hapus transaksi ini?", {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Laporan pemasukan & pengeluaran dana masjid
          </p>
        </div>
        <StatusBadge
          status="active"
          label={formatTimestamp(new Date().toISOString())}
        />
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <SummaryCard
            ...
            value={formatRupiah(summary.pemasukanBulanIni)}
            icon={<ArrowUpRight className="w-5 h-5" />}
          />
          <SummaryCard
            title="Pengeluaran Bulan Ini"
            value={formatRupiah(summary.pengeluaranBulanIni)}
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <select
            ...
            className="bg-white border border-gray-200 text-xs rounded-lg px-2.5 py-1.5 text-slate-700 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
          >
            {CATEGORIES.map((c) => (
          ...
          <input
            ...
            className="bg-white border border-gray-200 text-xs rounded-lg px-2 py-1.5 text-slate-700 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
          />
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="bg-white border border-emerald-200 hover:bg-emerald-50 text-emerald-800 text-xs font-bold px-4 py-2.5 rounded-xl transition flex items-center gap-1.5 cursor-pointer"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute inset-y-0 left-2 flex items-center text-xs text-slate-500 pointer-events-none">
                  Rp
                </span>
                <input
                  type="text"
              ...
              <p className="text-[9px] text-slate-400 mt-0.5">
                Otomatis format: {formAmountDisplay || "1.000.000"}
              </p>
            </div>
            <div>
              <label className="block text-[10px] font-bold text-emerald-900 mb-1">
          ...
          <table className="w-full text-left border-collapse min-w-[750px]">
            <thead>
              <tr className="bg-gray-100/60 border-b border-gray-200">
                <th className="px-5 py-4 text-xs font-bold uppercase tracking-wider text-gray-600">
                ...
                <th className="px-5 py-4 text-xs font-bold uppercase tracking-wider text-gray-600 text-right">
                  Jumlah
                </th>
                <th className="px-5 py-4 text-xs font-bold uppercase tracking-wider text-gray-600 text-center">
                  Tipe
                </th>
                <th className="px-5 py-4 text-xs font-bold uppercase tracking-wider text-gray-600 text-center">
                  Aksi
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 whitespace-nowrap">
                        <span
                          className={`px-2.5 py-0.5 rounded text-[10px] font-bold ${isIn ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}
                        >
                          {tx.category}
                        </span>
                      </td>
                      <td
                        className={`px-5 py-3.5 whitespace-nowrap text-sm font-bold font-mono text-right ${isIn ? "text-emerald-700" : "text-rose-600"}`}
                      >
                        {isIn ? "+" : "-"}
