...
function getPackageIdForDate(value: string): string {
  if (!value) return "pkg-weekdays";
  const [year, month, day] = value.split("-").map(Number);
  const dayOfWeek = new Date(Date.UTC(year, month - 1, day)).getUTCDay();
  return dayOfWeek === 0 || dayOfWeek === 6 ? "pkg-weekend" : "pkg-weekdays";
}

export default function BookingSection() {
  const [selectedPackage, setSelectedPackage] = useState("pkg-weekdays");
  const [selectedDate, setSelectedDate] = useState("");

  const handleSelectDate = (date: string) => {
    setSelectedDate(date);
    setSelectedPackage(getPackageIdForDate(date));
  };
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <p className="mt-3 text-base text-gray-600">
            Fasilitas ruang pertemuan megah bernuansa islami modern. Tersedia
            untuk resepsi pernikahan, haflatul ikhtitam, seminar nasional,
            wisuda tahfiz, serta rapat kerja kemasyarakatan.
          </p>
        </div>
            ...
            <div className="relative z-20 p-6 md:p-8 text-white space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-2">
                <div className="bg-white/10 backdrop-blur-xs p-3 rounded-xl border border-white/10">
                  <span className="block text-[10px] text-emerald-200 uppercase font-bold">
                    Luas Area Hall
                  </span>
                  <span className="text-sm font-bold font-mono">
                    {HALL_INFO.dimensions}
                  </span>
                </div>
                <div className="bg-white/10 backdrop-blur-xs p-3 rounded-xl border border-white/10">
                  <span className="block text-[10px] text-emerald-200 uppercase font-bold">
              ...
              <div className="flex items-center gap-2 text-xs text-amber-300">
                ...
                <span className="font-semibold">
                  Aturan Syariat Islami (Tidak bercampur baur berlebihan & menu
                  halal)
                </span>
              </div>
            </div>
          ...
          <div className="lg:col-span-5 bg-gray-50 rounded-2xl p-6 border border-gray-200 flex flex-col justify-between">
            ...
            <div className="mt-6 pt-4 border-t border-gray-200 text-center">
              <span className="text-[11px] text-gray-500 font-medium italic">
                * Keuntungan sewa dialokasikan penuh untuk kas pemeliharaan
        ...
        <div className="space-y-8 mb-12">
          <div className="text-center">
            <h3 className="text-2xl font-bold text-gray-900">
              Pilihan Paket Sewa Aula
            </h3>
            <p className="text-sm text-gray-500 mt-1">
              Kami menyediakan paket fleksibel sesuai anggaran dan format acara
              Anda.
            </p>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  <ul className="space-y-2.5 pt-2">
                    {pkg.features.map((feat, i) => (
                      <li
                        key={i}
                        className="flex gap-2 text-xs text-gray-600 leading-tight"
                      >
        ...
        <div className="mt-14 emerald-gradient text-white rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 border border-emerald-800 card-shadow relative overflow-hidden">
          ...
          <div className="space-y-2 relative z-10">
            ...
            <p className="text-xs md:text-sm text-emerald-200">
              Silahkan hubungi admin untuk konsultasi dan bertanya-tanya,
              Gratis!
            </p>
          </div>
          <a
...
function BookingRequestForm({
  ...
  const selectedPackageData =
    BOOKING_PACKAGES.find((pkg) => pkg.id === selectedPackage) ||
    BOOKING_PACKAGES[0];

  const handleDateChange = (value: string) => {
    setDate(value);
    onPackageChange(getPackageIdForDate(value));
  };
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setError("");
    setMessage("");
    if (!name || !whatsapp || !date || !purpose) {
      setError("Nama, WhatsApp, tanggal, dan keperluan wajib diisi.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      setMessage(
        "Permohonan booking tersimpan dengan status pending. WhatsApp admin akan terbuka untuk konfirmasi.",
      );
      window.open(
        `https://wa.me/${ADMIN_WA}?text=${encodeURIComponent(buildWhatsappTemplate())}`,
        "_blank",
        "noopener,noreferrer",
      );
      setName("");
    ...
    >
      <div>
        ...
        <p className="mt-1 text-xs text-gray-500">
          Data masuk ke dashboard admin sebagai pending, lalu lanjut konfirmasi
          melalui WhatsApp.
        </p>
      </div>
      {error && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600"
        ...
        <div>
          ...
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-900">
            {selectedPackageData?.name || "Weekdays (Senin-Jumat)"} -{" "}
            {selectedPackageData?.price || "Rp 8.000.000"}
          </div>
          <p className="mt-1 text-[10px] text-slate-500">
            Senin-Jumat otomatis Weekdays, Sabtu-Ahad otomatis Weekend.
          </p>
        </div>
        <div className="sm:col-span-2">
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600"
            placeholder="Contoh: Walimah, seminar, rapat keluarga"
          />
        </div>
        <div className="sm:col-span-2">
          <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-600 mb-1">
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600"
            placeholder="Opsional: detail acara, jumlah tamu, kebutuhan tambahan"
          />
        </div>
      </div>
      <button
        type="submit"
        disabled={submitting}
        className="w-full rounded-xl bg-emerald-700 hover:bg-emerald-800 disabled:opacity-60 text-white font-bold text-sm py-3 flex items-center justify-center gap-2"
      >
        {submitting ? (
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    days.push(
      <button
        ...
        className={`p-2 border rounded-lg text-center text-sm font-medium transition ${
          isBooked
            ? "bg-rose-100 text-rose-800 border-rose-200 cursor-not-allowed"
            : isSelected
              ? "bg-emerald-700 text-white border-emerald-700"
              : "bg-white text-gray-700 border-gray-100 hover:border-emerald-500 hover:text-emerald-700"
        }`}
      >
        {i}
      </button>,
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-bold text-gray-900">
            Kalender Booking Aula
          </h3>
          <p className="mt-1 text-xs text-gray-500">
            Pilih tanggal tersedia untuk mengisi form booking.
          </p>
        </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-6 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-600">
            Tidak tersedia / telah dibooking
          </span>
        </div>
      </div>
    </div>
