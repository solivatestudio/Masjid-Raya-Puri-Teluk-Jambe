...
import {
  ...
  Undo, Redo, Code, Strikethrough, AlignLeft, AlignCenter, AlignRight
} from 'lucide-react';
...
export default function ArticleEditor({ content, onChange, placeholder }: ArticleEditorProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-sm sm:prose-base max-w-none min-h-[400px] focus:outline-none p-4',
      },
    },
    onUpdate: ({ editor }) => {
  ...
  if (!editor) return <div className="border border-slate-200 rounded-xl p-4 bg-slate-50">Memuat editor...</div>;
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white">
      <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-slate-200 bg-slate-50">
        ...
        <button type="button" onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} className={btn(editor.isActive('heading', { level: 1 }))} title="Heading 1"><Heading1 className="w-4 h-4" /></button>
        ...
        <button type="button" onClick={() => editor.chain().focus().toggleOrderedList().run()} className={btn(editor.isActive('orderedList'))} title="Ordered List"><ListOrdered className="w-4 h-4" /></button>
        ...
        }} className={btn(editor.isActive('link'))} title="Link"><LinkIcon className="w-4 h-4" /></button>
        <button type="button" onClick={() => {
          const url = prompt('Image URL (kosongkan jika upload via UploadThing):');
          if (url) editor.chain().focus().setImage({ src: url }).run();
        }} className={btn(false)} title="Image URL"><ImageIcon className="w-4 h-4" /></button>
        <span className="w-px h-5 bg-slate-300 mx-1" />
