...
export default function PrayerTimes() {
  ...
  // Fixed coordinates approximation for beautiful UI rendering
  const basePrayerTimes: { name: string; timeStr: string; hour: number; minute: number }[] = [
  ...
  // Compute countdown and active state
  useEffect(() => {
    ...
    for (let i = 0; i < basePrayerTimes.length; i++) {
      ...
      // If it is after Isya, the next prayer is Subuh tomorrow
      if (i === basePrayerTimes.length - 1) {
        targetIndex = 0; // Subuh tomorrow
    ...
    // Active prayer is the one we are currently in
    let currentIndex = basePrayerTimes.length - 1; // Default is Isya
    for (let i = 0; i < basePrayerTimes.length; i++) {
      const p = basePrayerTimes[i];
      const pSecs = p.hour * 3600 + p.minute * 60;
